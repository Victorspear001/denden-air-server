/**
 * Denden Air — Dashboard Controller (Production / Cloudflare Edge)
 * Uses polling instead of SSE for Cloudflare Workers compatibility.
 */
(function () {
  'use strict';
  ...
  btnDeleteAll.addEventListener('click', async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/v1/sms${typeParam}`, {
        method: 'DELETE',
      ...
      if (res.ok) {
        ...
        if (activeTab === 'all') {
          ...
          knownIds = new Set(smsEntries.map((e) => e.id));
        }
  ...
  // ─── Fetch SMS Data (Polling) ──────────────────────────────
  async function pollSms() {
    try {
      const res = await fetch(`/api/v1/sms?limit=50&_t=${Date.now()}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      ...
      if (data.sms_logs && data.sms_logs.length > 0) {
        // Find new messages
        const newMessages = data.sms_logs.filter(sms => !knownIds.has(sms.id));

        if (newMessages.length > 0) {
          feedEmpty.classList.add('hidden');
          // Add oldest first so newest appear on top
          newMessages.reverse().forEach((sms) => {
            knownIds.add(sms.id);
            addSmsCard(sms, smsEntries.length > 0); // Only toast if not initial load
          });
        }
      }
      ...
      if (data.stats) {
        statDevices.textContent = data.stats.active_devices || 0;
      }
  ...
  function startPolling() {
    pollSms(); // Immediate first poll
    pollInterval = setInterval(pollSms, 1500);
  }
  ...
  function addSmsCard(sms, isNew) {
    ...
    card.innerHTML = `
      <div class="sms-card-header">
        ...
        <div style="display: flex; align-items: center; gap: 8px;">
          ...
          <button class="btn-delete-sms" data-id="${sms.id}" style="background: none; border: none; color: #ff4a4a; cursor: pointer; font-size: 16px; opacity: 0.7;" title="Delete message permanently">🗑️</button>
        </div>
      </div>
    ...
    // Add delete handler
    card.querySelector('.btn-delete-sms').addEventListener('click', async () => {
      if (confirm('Are you sure you want to permanently delete this message? This cannot be undone.')) {
        try {
          const res = await fetch(`/api/v1/sms/${sms.id}`, {
            method: 'DELETE',
            headers: { Authorization: `Bearer ${token}` }
          });
          if (res.ok) {
            card.remove();
            smsEntries = smsEntries.filter(e => e.id !== sms.id);
            knownIds.delete(sms.id);
            senderSet = new Set(smsEntries.map((e) => e.sender_identity));
            otpTotal = smsEntries.filter((e) => /\b\d{4,8}\b/.test(e.message_payload)).length;
            updateGlobalStats();
            filterMessages();
          ...
          console.error('[SMS] Delete failed:', err);
        }
      }
    });

    // Show toast for new messages (only after grace period)
    if (isNew && !suppressToasts) {
      showToast(sms);
      setTimeout(() => { card.classList.remove('new-entry'); }, 5000);
    } else if (isNew) {
      setTimeout(() => { card.classList.remove('new-entry'); }, 5000);
    }
  }
